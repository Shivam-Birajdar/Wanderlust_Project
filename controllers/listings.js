...
module.exports.createNewList = async (req, res, next) => {
  // database tasks are asynchronous , therefore for error handling wrapAsync is used.
...
module.exports.renderExistingList = async (req, res) => {
  ...
  if (!editingList) {
    // api protection when directly accessed through api
    req.flash("errorMsg", "list doesn't exist !");
    return res.redirect("/listings");
  }
  const OriginalPath = editingList.image.url.replace(
    "/upload",
    "/upload/h_200,w_250"
